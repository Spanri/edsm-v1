<template>
  <div id="grid">
    <div v-if="disable && id == 'notif'" style="color: red; margin-left: 50px; margin-top: 50px">
      Загрузка...
    </div>
    <div
      v-if="filteredHeroes.length == 0 && $route.params.id != 'common'"
    ...
    <div v-if="filteredHeroes.length != 0 || $route.params.id == 'common'" class="dataDiv">
      <form class="search">
        <p style="display: inline-block; margin: 0; margin-bottom: 15px; margin-right: 20px">
          Поиск по всем столбцам
        </p>
        <input placeholder="Введите что-то" name="query" v-model="filterKey" />
      </form>
      <div
        v-if="$route.params.id == 'common' && !disable"
        style="margin-left: 35px; margin-bottom: 10px"
      >
        <button :class="[commonActive == 1 ? 'commonButton' : '']" @click="commonActive = 1">
          ВСЕ ДОКУМЕНТЫ
        </button>
        <button :class="[commonActive == 2 ? 'commonButton' : '']" @click="commonActive = 2">
          МОИ ДОКУМЕНТЫ
        </button>
        <button :class="[commonActive == 3 ? 'commonButton' : '']" @click="commonActive = 3">
          НЕ МОИ ДОКУМЕНТЫ
        </button>
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr
            v-for="(entry, j) in filteredHeroes"
            :key="j"
            :style="styleObject(entry)"
            class="rowData"
...
export default {
  ...
  created() {
    this.$store.dispatch(DOCS_REQUEST).then((t) => {
  ...
  computed: {
    ...
    heroes() {
      ...
      let output = this.getDocs.filter((d) => d.status != 2 && d.status != 3 && d.status != 7);
      if (id == "all") {
        output = output;
      } else if (id == "common") {
        ...
        output = this.getDocs.filter((d) => d.user.id == this.getProfile.id && d.status == 0);
        ...
        output = this.getDocs.filter((d) => d.status == 5);
      } else if (id == "my-doc-signature-request") {
        output = this.getDocs.filter(
          (d) => d.user.id == this.getProfile.id && d.status == 0 && !d.doc.signature_end
        );
        // console.log(output[0].initiator.profile.id)
      } else if (this.id == "notif") {
        let docs = this.getDocsOld.filter((d) => d.status == 2 || d.status == 3 || d.status == 7);
        docs.forEach((d) => {
          if (d.status == 2) {
