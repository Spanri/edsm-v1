      ...
      <div style="margin-left: 25px">
        <form @submit.prevent="addDoc">
          ...
          <select v-model="file_cabinet">
            <option v-for="(fileC, i) in fileCabinets" :key="i" :value="fileC">
              {{ fileC.name }}
            </option>
          </select>
          <div
            style="border: 1px solid #347090; border-radius: 3px; padding: 15px; margin-top: 10px"
          >
            <p style="margin-top: 0px">
              Запросить подпись. Порядок выбранных пользователей можно менять перетаскиванием.
            </p>
            <draggable v-model="selectedUsers">
              <div v-for="user in selectedUsers" :key="user.id" class="selectedUserWithCancel">
                ...
                <svg
                  ...
                  @click="deleteSelectedUser(1, user)"
                  height="21px"
                  viewBox="0 0 31 32"
                  width="22px"
                  xml:space="preserve"
            ...
            <button type="button" @click="addUser(1)">ДОБАВИТЬ</button>
            <br />
          </div>
          <div
            style="border: 1px solid #347090; border-radius: 3px; padding: 15px; margin-top: 10px"
          >
            <p style="margin-top: 0">
              Пользователи, у которых были запрошены подписи, могут просматривать документ. Ниже
              можно указать дополнительных пользователей, которые не должны подписывать документ, но
              могут его просматривать. Порядок выбранных пользователей можно менять перетаскиванием.
            </p>
            <draggable v-model="selectedUsers2">
              <div v-for="user in selectedUsers2" :key="user.id" class="selectedUserWithCancel">
                ...
                <svg
                  class="deleteSelectedUser"
                  @click="deleteSelectedUser(2, user)"
                  height="21px"
                  viewBox="0 0 31 32"
                  width="22px"
                  xml:space="preserve"
            ...
            <button type="button" @click="addUser(2)">ДОБАВИТЬ</button>
            <br />
          </div>
          <div style="height: 15px"></div>
          <p>Описание</p>
          <textarea v-model="description" placeholder="Опишите документ"></textarea>
          <p></p>
          Общий доступ
          <input type="checkbox" name="common" true-value="1" false-value="0" v-model="common" />
          <div style="height: 35px"></div>
          <button type="submit" :class="{ disabled: this.disable }" style="margin-bottom: 50px">
            СОЗДАТЬ
          </button>
          <br />
        </form>
...
export default {
  ...
  created() {
    this.$store.dispatch(USERS_EMAILS).then((resp) => {
  ...
  methods: {
    ...
    addDoc() {
      ...
      // для файла
      if (this.file) {
        const newFile = new File([this.file], this.title + "." + this.typeFile, {
          type: this.file.type
        });
