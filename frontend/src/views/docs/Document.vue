<template>
  <div id="document">
    ...
    <div v-else>
      <div class="buttons">
        <button
          v-if="doc.status == 0 && doc.user.id == this.$store.getters.getProfile.id"
          @click="editDoc()"
        >
          РЕДАКТИРОВАТЬ ИНФОРМАЦИЮ О ФАЙЛЕ
        </button>
        <button
          v-if="doc.status == 0 && doc.user.id == this.$store.getters.getProfile.id"
          @click="isRepeatSign = true"
        >
          ИЗМЕНИТЬ ФАЙЛ/ПЕРЕЗАПУСТИТЬ ЦЕПОЧКУ ПОДПИСЕЙ
        </button>
      </div>
      <div class="document2Colon">
        <div>
          ...
          <button
            style="margin-top: 20px; margin-left: 6px; background: rgb(243, 92, 92)"
            v-if="
              this.$store.getters.getProfile.is_staff ||
              (doc.status == 0 &&
                (doc.doc.common != true || doc.user.id == this.$store.getters.getProfile.id))
            "
            @click="deleteDoc()"
          >
            УДАЛИТЬ
        ...
        <div style="margin-left: 25px; margin-top: 0px">
          ...
          <p v-if="doc.doc.cancel_description" style="color: red">
            Ваш документ отказались подписывать. Посмотрите комментарий отказа и нажмите кнопку
            "Запустить цепочку подписей снова". Выберите в появившемся окне новый документ
            (необязательно) и нажмите "Подтвердить", чтобы запустить цепочку подписей.
          </p>
          <p v-if="doc.doc.cancel_description">
            Комментарий отказа: {{ doc.doc.cancel_description }}
          </p>
          ...
          <table class="aboutDoc">
            ...
            <tbody>
              ...
              <tr>
                <td>Файл</td>
                <td>
                  <button v-if="doc.doc.size / 1024 / 1024 < 25" @click="viewDoc()">
                    СМОТРЕТЬ
                  </button>
              ...
              <tr>
                ...
                <td>
                  ...
                  <button v-if="doc.status == 2" @click="isConfirm = true">
                    ПОДПИСАТЬ/ОТКЛОНИТЬ
                  </button>
                  <br />
                  {{ doc.doc.signature }}
          ...
          <div v-if="isConfirm" class="confirm">
            ...
            <div class="signButtons" style="margin-top: 5px">
              ...
              <button
                @click="
                  ...
                  isPreConfirm = false;
                "
                style="background: rgb(243, 92, 92)"
              >
                ОТКЛОНИТЬ
              </button>
            ...
            <p
              v-if="errorAddSign"
              style="color: red; text-align: center; padding: 5px; width: 100%"
            >
              {{ errorAddSign }}
            </p>
          </div>
          <div
            ...
            class="confirm"
            style="height: 380px"
            @submit.prevent="cancelSign"
          >
            <div style="display: grid; grid-template-columns: auto max-content">
              <div></div>
              <svg
                @click="isCancelSign = false"
                style="margin-right: 10px; cursor: pointer; margin-top: 7px"
                height="22px"
                viewBox="0 0 33 33"
                width="22px"
            ...
            <textarea v-model="cancelCause" placeholder="Введите комментарий"></textarea>
            <input
              type="file"
              id="file"
              name="file"
            ...
            <p v-if="errorConfirm" style="color: red; text-align: center; padding: 5px">
              {{ errorConfirm }}
            </p>
          </div>
          <div v-if="isRepeatSign" class="confirm">
            ...
            <p style="margin: 0">Загрузите новый файл (необязательно) и нажмите "ПЕРЕЗАПУСТИТЬ".</p>
            <input
              type="file"
              id="file"
              name="file"
...
export default {
  ...
  created() {
    this.refresh = true;
    this.$store.dispatch(DOCS_REQUEST).then((s) => {
  ...
  methods: {
    ...
    cancelSign() {
      ...
      this.$store
        .dispatch(DOC_SIGNATURE_CANCEL, {
          id: this.id,
          data: d
        })
        .then((resp) => {
          this.refresh = true;
          this.$store.dispatch(DOCS_REQUEST).then((r) => {
            this.doc = this.$store.getters.getDoc(this.id);
            this.error =
              "Документ отклонен. Через несколько секунд вы перейдете на главную страницу.";
    ...
    checkSign() {
      ...
      this.$store.dispatch(DOC_SIGNATURE_CHECK, this.id).then((resp) => {
