  ...
  <div id="menu">
    ...
    <div v-if="!closeMenu" class="menuOpen">
      <div title="Скрыть" style="display: grid; grid-template-columns: 1fr auto">
        ...
        <svg
          class="openCloseMenuButton"
          style="margin-bottom: 0px; margin-top: 15px; margin-right: 15px"
      ...
      <select v-model="fileCabinet" @change="onChange()">
        <option v-for="fileC in fileCabinets" :key="fileC.id" :value="fileC">
          {{ fileC.name }}
        </option>
      </select>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/all">
        ВСЕ ДОКУМЕНТЫ
      </router-link>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/myDocs">
        МОИ ДОКУМЕНТЫ
      </router-link>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/common">
        ОБЩИЙ ДОСТУП
      </router-link>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/available-to-me">
        ДОСТУПНЫ МНЕ
      </router-link>
      <router-link
        @click.native="goToFolder"
        class="router-link"
        to="/documents/my-doc-signature-request"
      >
        НА ПОДПИСИ
      </router-link>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/signature-request">
        НА ПОДПИСЬ
      </router-link>
      <router-link @click.native="goToFolder" class="router-link" to="/documents/signature-success">
        ПОДПИСАННОЕ
      </router-link>
    </div>
  </div>
