<template>
  <div v-if="newPassword == 1" class="remember-password">
    <p class="remember-password__title">СМЕНА ПАРОЛЯ - ШАГ 1</p>

    <p class="remember-password__text" style="">
      Это страница смены пароля. Чтобы подтвердить свою почту, впишите ее в
      поле. Вам придет код на почту, который следует ввести на следующем шаге.
    </p>
  ...
  <div v-else-if="newPassword == 2" class="remember-password">
    <p class="remember-password__title">СМЕНА ПАРОЛЯ - ШАГ 2</p>

    <p class="remember-password__text">
    ...
    <button type="submit" @click="updatePassword()" style="margin-left: 5px">
      ПОДТВЕРДИТЬ
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    rememberPassword() {
      ...
      this.$store
        .dispatch(USER_CONFIRM_UPDATE_PASSWORD, emailForConfirm)
        .then((resp) => {
          this.error = null;
        ...
        .catch((err) => {
          ...
          if (err.length > 200) {
            if (
              err.substring(0, 39) == "NameError at /rest_auth/password/reset/"
            ) {
    ...
    updatePassword() {
      ...
      this.$store
        ...
        .catch((err) => {
          console.log(err);
          if (
            err == '{"uid":["Invalid value"]}' ||
            err == '{"token":["Invalid value"]}'
          ) {
